 ...
 * @since 1.0.0
 */
public class DbEntitySessionImplInstance<E extends EntityBean> extends AbstractDbEntitySession<E> {
   ...
   * The constructor.
   *
   * @param readOnly to create a read-only copy for {@link DbEntityHolder#getExternal() external entities},
   *        {@code false} otherwise (mutable copy).
   */
  public DbEntitySessionImplInstance(boolean readOnly) {
  ...
  protected DbEntityHolder<E> createHolder(E managed) {
    ...
    E external = ReadableBean.copy(managed, this.readOnly);
    return new DbEntityHolderStatic<>(managed, external);
  }
